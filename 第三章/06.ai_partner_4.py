    ...
    try:
        if os.path.exists(f"session/{session_name}.json"):
            ...
            if session_name == st.session_state.current_session:
                ...
                st.session_state.nick_name = "祺祺"
                st.session_state.nature = "活泼可爱的福建萌妹子"
    except Exception as e:
...
# nick_name = st.sidebar.text_input("呢称")
with st.sidebar: # with: 创建一个上下文管理器, 在with代码块内, 可以使用with中的变量
    st.subheader("伴侣信息")
    # 会话信息
    st.subheader("AI控制面板")
    ...
    # 昵称输入框
    nick_name = st.text_input("呢称", placeholder="请输入昵称", value=st.session_state.nick_name)
    if nick_name:
...
if prompt: # 字符串会自动转化为布尔值, 如果字符串不为空, 则为True, 否则为False
